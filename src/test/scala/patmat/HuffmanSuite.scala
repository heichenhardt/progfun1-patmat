...
class HuffmanSuite extends FunSuite {
  ...
  test("until of some leaf list") {
    val leaflist = List(Leaf('e', 1), Leaf('t', 2), Leaf('x', 4))
    assert(until(Huffman.singleton, Huffman.combine)(leaflist) === List(Fork(Fork(Leaf('e',1),Leaf('t',2),List('e', 't'),3),Leaf('x',4),List('e', 't', 'x'),7)))
  }
  ...
  test("decode") {
    ...
    assert(decode(tree, List(1,0,0,0,1)) === List('x', 'e', 't'))
  }


  test("decode and encode a very short text should be identity") {
    new TestTrees {
      assert(decode(t1, encode(t1)("bbabaaaa".toList)) === "bbabaaaa".toList)
  ...
  test("Code table") {
    new TestTrees {
      assert(codeBits(convert(frenchCode))('h') === List(0,0,1,1,1,0,1))
    }
  }

  test("decode and quickEncode a very short text should be identity") {
    new TestTrees {
      assert(decode(t1, quickEncode(t1)("bbabaaaa".toList)) === "bbabaaaa".toList)
